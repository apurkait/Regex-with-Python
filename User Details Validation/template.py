...
while True:
    ...
    check = re.fullmatch(r"(0[1-9]|1[0-2])/(0[1-9]|[12]\d|3[01])/(0\d\d[1-9]|1\d{3}|200[01])", date)
...
while True:
    ...
    check = re.fullmatch(r"^[a-z](?=.{7,})(?=.*[A-Z])(?=.*\d)(?=.*[$&?!%])[a-zA-Z\d$&?!%]+$", passw)
...
while True:
    ...
    check = re.fullmatch(r"(0[5-9]|1[0-2])/24|(0[1-9]|1[0-2])/(2[5-9]|[3-9]\d)", ccdat)
